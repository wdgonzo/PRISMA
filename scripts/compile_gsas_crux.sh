...
# Step 4.5: Create numpy.pc for pkg-config
echo "Step 4.5: Creating numpy.pc for meson dependency detection..."
echo "-----------------------------------------------------------------------"

# Get NumPy information
NUMPY_VERSION=$(python3 -c 'import numpy; print(numpy.__version__)')

# Create pkgconfig directory in venv
PKGCONFIG_DIR="${VENV_PATH}/lib/pkgconfig"
mkdir -p "${PKGCONFIG_DIR}"

# Create numpy.pc file
NUMPY_PC="${PKGCONFIG_DIR}/numpy.pc"
cat > "${NUMPY_PC}" << EOF
# pkg-config file for NumPy
# Generated by compile_gsas_crux.sh

Name: numpy
Description: NumPy - Numerical Python library
Version: ${NUMPY_VERSION}
Cflags: -I${NUMPY_INCLUDE}
EOF

echo "Created: ${NUMPY_PC}"
echo "Contents:"
cat "${NUMPY_PC}"
...
# Verify pkg-config can find numpy
echo ""
echo "Testing pkg-config detection..."
if pkg-config --exists numpy; then
    echo "  ✓ pkg-config can find numpy"
    echo "  Version: $(pkg-config --modversion numpy)"
    echo "  Cflags: $(pkg-config --cflags numpy)"
else
    echo -e "${RED}  ✗ pkg-config cannot find numpy${NC}"
    echo "  This may cause meson setup to fail"
fi

echo -e "${GREEN}✓ numpy.pc configured${NC}"
echo ""
